...
public class Exercise2Test extends PetDomainForKata
{
    ...
    {
        Predicate<Person> predicate = person -> person.getFirstName().equals("Mary") &&
               person.getLastName().equals("Smith");
        Person result = this.people.select(predicate).getFirst();
    ...
    {
        Predicate2<Person, Integer> hasEnoughPet =
              (person, numberOfPets) -> person.getNumberOfPets() >= numberOfPets;
        MutableList<Person> petPeople = this.people.selectWith(hasEnoughPet, 1);
